class TasksController < ApplicationController
  ...
  #          WorkingOnTask.new(user_id: user_id, task_id: already_did_this_shit(user_id).map(&:task_id).first, start_time: calculate_time_from_interval('start', already_did_this_shit(user_id).map(&:task_id).first), end_time: calculate_time_from_interval('end', already_did_this_shit(user_id).map(&:task_id).first)).save
  #        else
